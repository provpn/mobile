...
import (
	...
	"github.com/provpn/mobile/bind"
	"github.com/provpn/mobile/internal/importers"
	"github.com/provpn/mobile/internal/importers/java"
	"github.com/provpn/mobile/internal/importers/objc"
...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named) {
	...
	case "java":
		...
		if p == nil {
			dir, err := packageDir("github.com/provpn/mobile/bind")
			if err != nil {
				errorf(`"github.com/provpn/mobile/bind" is not found; run go get github.com/provpn/mobile/bind: %v`, err)
				return
			}
			repo := filepath.Clean(filepath.Join(dir, "..")) // github.com/provpn/mobile directory.
			...
			javaDir, err := packageDir("github.com/provpn/mobile/bind/java")
			if err != nil {
				errorf("unable to import bind/java: %v", err)
				return
			}
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			dir, err := packageDir("github.com/provpn/mobile/bind/objc")
			if err != nil {
