...
func runBuildImpl(cmd *command) (*packages.Package, error) {
	...
	if !nmpkgs["github.com/provpn/mobile/app"] {
		return nil, fmt.Errorf(`%s does not import "github.com/provpn/mobile/app"`, pkg.PkgPath)
	}
...
func extractPkgs(nm string, path string) (map[string]bool, error) {
	if buildN {
		return map[string]bool{"github.com/provpn/mobile/app": true}, nil
	}
	r, w := io.Pipe()
...
func parseBuildTarget(buildTarget string) (os string, archs []string, _ error) {
	...
	for i, p := range strings.Split(buildTarget, ",") {
		...
		if osarch[0] != "android" && osarch[0] != "ios" && osarch[0] != "darwin" {
