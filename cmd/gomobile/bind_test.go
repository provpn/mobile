...
func TestBindAndroid(t *testing.T) {
	...
	for _, tc := range tests {
		...
		cmdBind.flag.Parse([]string{"github.com/provpn/mobile/asset"})
		err := runBind(cmdBind)
		if err != nil {
			t.Log(buf.String())
			t.Fatal(err)
...
WORK=$WORK
GOOS=android CGO_ENABLED=1 gobind -lang=go,java -outdir=$WORK{{if .JavaPkg}} -javapkg={{.JavaPkg}}{{end}} github.com/provpn/mobile/asset
mkdir -p $WORK/src
...
WORK=$WORK
GOOS=darwin CGO_ENABLED=1 gobind -lang=go,objc -outdir=$WORK -tags=ios{{if .Prefix}} -prefix={{.Prefix}}{{end}} github.com/provpn/mobile/asset
mkdir -p $WORK/src
...
func TestBindWithGoModules(t *testing.T) {
	...
	if out, err := exec.Command("go", "build", "-o="+dir, "github.com/provpn/mobile/cmd/gobind").CombinedOutput(); err != nil {
		t.Fatalf("%v: %s", err, string(out))
	}
	if out, err := exec.Command("go", "build", "-o="+dir, "github.com/provpn/mobile/cmd/gomobile").CombinedOutput(); err != nil {
		t.Fatalf("%v: %s", err, string(out))
	}
	path := dir
	...
	for _, target := range []string{"android", "ios"} {
		t.Run(target, func(t *testing.T) {
			...
			tests := []struct {
				...
				{
					Name: "Absolute Path",
					Path: "github.com/provpn/mobile/bind/testdata/cgopkg",
				},
				{
					Name: "Relative Path",
